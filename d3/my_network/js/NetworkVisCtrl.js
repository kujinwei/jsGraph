...
//TODO we should use directive to draw a zone topology, currently we use angular controller
appCtrl.controller('NetworkVisCtrl',['$scope', '$q' , '$timeout', '$modal','$d3popover', 'NetworkService', 'CloudComponents',
    function($scope, $q , $timeout , $modal , $d3popover , NetworkService , CloudComponents){
        ...
        var STATUS = {healthy : "healthy" , unhealthy:"unhealthy"  } ;
        var LAYER = {zone:"zone" , vlan:"vlan" , subnet:"subnet" , chart:"chart"} ;
        ...
        var init = function() {
            ...
            svg = d3.select("#NetworkDiagram").append("svg")
                .attr("border",1);
...
//            svg.call(drag) ;
        ...
        var showLoading = function() {
            loadingBox = svg.append("text")
                .text("Loading the data, please wait ...")
                .attr('text-anchor','middle')
                .attr('x',width/2)
                .attr('y',height/2 + 2)
                .attr('font-size', '12');
            ...
            for (var i = 0; i< vlans.length ; i++) {
                ...
                if (vlan.number == source.vlan && source.result == "unpingable") {
                    ...
                    if (currentLayer().type == LAYER.zone) {
                        marker = g.append('image')
                            .attr({
                                width: 20,
                                height: 20
                            })
                            .attr('x',vlan.pos.left + vlan.pos.width/2 - cellSize/2)
                            .attr('y',vlan.pos.top + cellSize)
                            .attr('xlink:href',function(d){
                ...
                for (var j = 0; j < vlan.subnets.length ; j++) {
                    ...
                    if (inSubnet(vm , subnet) && source.result == "unpingable") {
                        ...
                        if (angular.isDefined(pos) && (currentLayer().type == LAYER.zone) ) {
...
//                    }
                    ...
                    lineData.push({'startTime':new Date().addHours(r) , 'endTime': new Date().addHours(r+2) , 'value':1}) ;
                }
                ...
                NetworkService.loadHistory(instanceIP , function (result) {
                    ...
                    lineData.forEach(function(d){
                        ...
                        lineData.value = 1 ; //pingable and unpingable are in the same line
                    }) ;
        ...
        var loadCloudComponents = function() {
            if (debug) {
                ...
                CloudComponents.get(function(data){
                    componentMap = data.list;
                    mapComponentsToInstances() ;
                    hideLoading() ;
                    draw() ;
                    checkPing() ;
        ...
        var checkPing = function() {
            if (debug) {
                d3.json("data/ping.json" , function(json){
                    //redraw() ;
                    var pings = angular.copy(json.data) ;
                    for (p in pings) {
                ...
                NetworkService.checkPing(function (result) {
                    handlePing(result.data) ;
                    $timeout(checkPing, 5000);
                }) ;
            }
...
//        .attr('font-family', 'sans-serif')
            ...
            if (align === "left") {
                bone = g.append("line")
                    .attr("x1" , pos.left + 70)
                    .attr("y1" , y)
                    .attr("x2" , pos.right - 10)
                    .attr("y2" , y)
                    .attr("stroke" , "gray")
                    .attr("stroke-width" , "6");
                firewall = g.append('image')
                    .attr({
                        width: 40,
                        height: 40
                    })
            ...
            if (align === "left") {
                g.append("line")
                    .attr("x1" , bone.position().left+4)
                    .attr("y1" , y)
                    .attr("x2" , firewall.position().right-4)
                    .attr("y2" , y)
                    .attr("stroke" , "black")
                    .attr("stroke-width" , "1");
            } else if (align === "right") {
            ...
            for (var j = 0; j < cols && i < vlan.subnets.length ; j++) {
                ...
                i ++ ;
                g.append("line")
                    .attr("x1" , subnet.position().left + cellSize/2 )
                    .attr("y1" , subnet.position().bottom-4)
                    .attr("x2" , subnet.position().left + cellSize/2)
            ...
            if (vlan.subnets.length > cols*2) {
                g.append('text')
                    .text("...")
                    .attr('text-anchor','middle')
                    .attr('x',_x + cellSize + cellDistance + 2)
                    .attr('y',_y + cellSize/2)
                    .attr('stroke' ,'blue')
                    .attr('stroke-width' , 1)
                    .attr('font-size', '14')
...
//            textWidth = bbox.width ;
            compLabel.attr('x',vmEle.position().left + (vmEle.position().width)/2 )
                .attr('y',vmEle.position().bottom + 7) ;
        ...
        var drawSubnet = function(subnet) {
            ...
            //add the label
            g.append("text")
                .text("Instances for subnet : " + subnet.ip)
                .attr('text-anchor','middle')
        ...
        var drawVlan = function(vlan) {
            ...
            //add the label
            g.append("text")
                .text("VLAN : " + vlan.number + " and its subnet")
                .attr('text-anchor','middle')
                .attr('x', width/2 - xOffset)
                .attr('y', barHeight )
                .attr('stroke' ,'gray')
                .attr('stroke-width' , 1) ;
            //draw the firewall
            var firewall = g.append('image')
                .attr({
                    width: cellSize,
                    height: cellSize
                })
                .attr('x',width/2 - cellSize/2)
                .attr('y', barHeight+10)
                .attr('xlink:href',function(d){
                    return 'images/network/firewall.png';
                });
            ...
            g.append("line")
                .attr("x1" , firewall.position().left + firewall.position().width/2 )
                .attr("y1" , firewall.position().bottom)
                .attr("x2" , firewall.position().left + firewall.position().width/2)
                .attr("y2" , _y - cellDistance)
                .attr("stroke" , "black")
                .attr("stroke-width" , "2");
            ...
            for (var i = 0; i < vlan.subnets.length ; i++) {
                ...
                if(i % cols == 0) {
                    g.append("line")
                        .attr("x1" , xOffset + cellSize/2 )
                        .attr("y1" , _y - cellDistance)
                ...
                g.append("line")
                    .attr("x1" ,_x + cellSize/2+1)
                    .attr("y1" , _y+7)
                    .attr("x2" , _x + cellSize/2+1)
                    .attr("y2" , _y-cellDistance)
                    .attr("stroke" , "black")
                    .attr("stroke-width" , "1");
                //link to above line
                if (i >= cols) {
                    var offset = 6 ;
                    g.append("line")
                        .attr("x1" ,ele.position().right + cellDistance/2 - offset)
                        .attr("y1" , _y-cellDistance)
                        .attr("x2" , ele.position().right + cellDistance/2 - offset)
                        .attr("y2" , _y-cellDistance*2)
                        .attr("stroke" , "black")
                        .attr("stroke-width" , "1");
                }
        ...
        var linkFirewall = function() {
            ...
            for (var i = 0; i< firewalls.length && (i+1) < firewalls.length ; i=i+2) {
                ...
                g.append("line")
                    .attr("x1" , firewall1.pos.right-2)
                    .attr("y1" , firewall1.pos.top + firewall1.pos.width/2+2)
                    .attr("x2" , firewall2.pos.left+4)
                    .attr("y2" , firewall2.pos.top + firewall2.pos.width/2+2)
                    .attr("stroke" , "green")
                    .attr("stroke-width" , "1");
            }
            for (var i = 0; i< firewalls.length && (i + 2) < firewalls.length; i++) {
                ...
                g.append("line")
                    .attr("x1" , firewall1.pos.left + firewall1.pos.width/2)
                    .attr("y1" , firewall1.pos.bottom-4)
                    .attr("x2" , firewall2.pos.left + firewall2.pos.width/2+2)
                    .attr("y2" , firewall2.pos.top+4)
                    .attr("stroke" , "green")
                    .attr("stroke-width" , "1");
            }
        } ;
        ...
        var popover = function(x,y) {
            $modal.open({
                templateUrl: '/template/d3/popover.html',
        ...
        function zoomed() {
            ...
            var scale = 1 ; //don't use d3.event.scale so far
        ...
        var showSubnetInfo = function(subnetId , subnet) {
            //angular.element(subnetEle)
//            var ele = angular.element(document.querySelector("#" + subnetId)) ;
//            d3Popover = $d3popover(ele, {title: 'Title', content: 'He!', placement: 'right'});
//            showTooltip() ;

            var modalInstance = $modal.open({
                templateUrl: '/template/taishan/modal/target_vm_detail_modal.html',
...
var SubnetCtrl = function($scope, $modalInstance) {
